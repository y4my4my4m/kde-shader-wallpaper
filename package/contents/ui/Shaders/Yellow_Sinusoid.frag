...
// Yellow Sinusoid — golden traveling sine on black, glow + 2nd harmonic
...
void mainImage(out vec4 C, in vec2 U)
{
    ...
    vec2 uv = (U - .5*R)/R.y;              // centered, aspect-correct
    float t = iTime;
    uv += fogWarp(uv, t);
    ...
    // main traveling sine
    float k = 6.28318/1.4;                 // spatial frequency
    float y = .30*sin(k*uv.x - 1.2*t);
    float d = abs(uv.y - y);
    col += vec3(1.00,.84,.20) * (.012/(d + .004));      // hot core
    col += vec3(1.00,.65,.05) * .010/(d*d*30. + .022);  // warm halo, tight

    // faint second harmonic drifting the other way
    float y2 = .18*sin(2.*k*uv.x + .8*t + 1.7);
    float d2 = abs(uv.y - y2);
    col += vec3(.80,.55,.10) * .0022/(d2 + .012);

    // keep edges black
    col *= smoothstep(1.15, .45, length(uv*vec2(.7,1.25)));
