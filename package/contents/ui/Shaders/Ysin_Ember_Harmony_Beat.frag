...
// settles at 122.2 +- 3.7 BPM with the phase locked to the true onsets at a
// circular concentration of 0.86. The flow rate still follows LOUDNESS, not
// tempo - the bar grid in the table below is still nominal - so the tracker
// drives the lift and nothing else. That is on purpose: letting a tempo
// estimate steer the flow would make every mis-detection a visible lurch.
//
// _10 also carries an IDLE STATE for when there is no audio at all - not a
// quiet passage, but silence, a frozen capture or nothing connected. After
// ~6 s of a spectrum that has stopped changing, three slow streams of value
// noise take over the harmonics and the line goes on breathing on its own,
// with no beat in it by construction. Simulated on a real track spliced to a
// frozen input: idle stayed at 0.000 throughout the music, reached half after
// 7.1 s of stillness, and fell back under 0.1 within 0.92 s of sound
// returning. Details at the block in the main pass and the detector in the
// buffer.
//
// ---------------------------------------------------------------------------
// _09 - every rate recomputed instead of guessed.
...
// The paired Ysin_Ember_Harmony_Beat_bufferA.frag holds the bands, their AGC
...
void mainImage(out vec4 C, in vec2 U){
    ...
    float treb = texture(iChannel1, vec2(17./28., .5)).w;   // main-wave sparkle, unchanged

    // integrator state from buffer A (six columns; see that file)
    // Fourteen columns since _10 (sub-bass, deepness, braid phases, beat).
    vec4 sA  = texture(iChannel1, vec2( 1./28., .5));
    vec4 sB  = texture(iChannel1, vec2( 3./28., .5));
    vec4 m1  = texture(iChannel1, vec2(11./28., .5));   // slow means b4,b5
    ...
    vec4 sd0 = texture(iChannel1, vec2(15./28., .5));   // drives b0..b3
    vec4 sd1 = texture(iChannel1, vec2(17./28., .5));   // drives b4,b5
    float Es = sB.b;
    // percussion pulses: kick leads, snare/hats add a lighter tick
    vec4 sP = texture(iChannel1, vec2(13./28., .5));
    vec4 sQ = texture(iChannel1, vec2(19./28., .5));   // low-bass onset (_06)
    vec4 sR = texture(iChannel1, vec2(21./28., .5));   // its deepness (_06)
    ...
    // --- IDLE: the line breathing with no audio at all ----------------------
    // With nothing playing every band drive is zero, the gate is shut and the
    // wave collapses to a nearly straight line - correct, and dead to look at.
    // 'idle' (see the buffer) reaches 1 only after ~6 s of a spectrum that has
    // stopped changing, i.e. silence, a frozen capture or no stream at all.
    //
    // What replaces the audio is three independent streams of value noise, on
    // purpose rather than one scaled three ways: the harmonics must not share
    // a period, or the line acquires exactly the pulse this is meant to avoid.
    // Their time scales (.13/.097/.071) are mutually irrational-ish for the
    // same reason, and they are slow - roughly 5 to 10 s per excursion.
    //
    // The line ends up swinging about +-60 px on a 1440p screen, drifting
    // across the screen in ~10 s (the flow rate falls to its .24 floor when Es
    // is zero, which is exactly the calm the idle state wants). Raise the .45
    // /.38/.30 spans for a livelier sleep, the constants beside them for a
    // deeper resting shape.
    float idle = clamp(sR.z, 0., 1.);
    if (idle > .001) {
        float w1 = .5 + .5*fnoise3(vec3( 3.1,  1.7, .130*t));
        float w2 = .5 + .5*fnoise3(vec3(-2.4,  5.2, .097*t));
        float w3 = .5 + .5*fnoise3(vec3( 7.8, -3.3, .071*t));
        lo = mix(lo, .30 + .45*w1, idle);
        md = mix(md, .22 + .38*w2, idle);
        hi = mix(hi, .15 + .30*w3, idle);
        // the swing factor too, or the noise only reshapes a line that is
        // still held at its quiet size by press = 0
        press = mix(press, .12 + .20*w2, idle);
    }
    // Overall size of the idle wave. Without this the line woke up at about
    // 60 px of swing on a 1440p screen, which is a shape change rather than
    // something happening; x2 puts it near 125 px - clearly alive, still well
    // under the braid's middle strand, and it costs nothing while audio plays
    // because idle is 0 there.
    // Modelled peak swing of the idle line, at noise = its mean value:
    //   gain 2.0 -> .257 (185 px)   3.0 -> .370 (267 px)   4.0 -> .468 (337 px)
    // against a braid that reaches about .27 (195 px) in the same state. 2.5
    // puts the line just level with the strands it lies in. Note the drives
    // are ALSO scaled by idle, so the response is quadratic in it - at a third
    // of full idle this whole block is worth about 48 px, which is what a
    // half-triggered detector looked like on screen.
    float idleGain = 1. + 1.5*idle;
    ...
    // --- _10: THE BEAT, AS A LIFT ------------------------------------------
    // The whole line - and the flame standing on it - rides a little higher
    // when the passage has bass, and that height breathes on the tracked beat.
    // It is deliberately the calmest gesture available: no extra wobble, no
    // brightness flash, just the wave sitting up.
    //
    // Three factors, and all three have to be there for anything to show:
    //   bassPresence  a ~4 s average of the sub-bass detector, so a track with
    //                 no low end never lifts at all;
    //   beatConf      the tracker's own confidence, so an unsteady passage
    //                 fades the effect out instead of guessing;
    //   beatEnv       a raised cosine on the predicted beat phase, ^1.5 so the
    //                 peak is narrower than the trough - a nudge, not a bob.
    //
    // Applied AFTER the limiter on purpose: the lift is a baseline shift, not
    // part of the waveform, and squashing it through the tanh would make it
    // shrink exactly when the wave is already large. Measured on a 120 BPM
    // track it is 6 px at the median and 28 at the 90th percentile on a 1440p
    // screen, peaking near 44 - visible as breathing, invisible as motion.
    // .08 is the knob; .05 makes it a hint, .12 starts to read as bouncing.
    float beatEnv  = pow(.5 + .5*cos(6.2831853*sT.z), 1.5);
    float beatConf = clamp(sT.w, 0., 1.);
    float bassPres = clamp(sR.y*2.5, 0., 1.);
    y += .08 * beatConf * bassPres * beatEnv;
    ...
    for (int i = 0; i < 6; i++){
        ...
        // The braid gets a small share of the idle wander too. Strictly the
        // request was about the line, but a still braid beside a moving line
        // reads as a broken scene rather than a calm one. Independent noise
        // per strand, so they drift apart instead of pulsing together.
        b = mix(b, .16 + .26*(.5 + .5*fnoise3(vec3(11. + float(i)*3.7, 2.3, .085*t))), idle);
