...
// Ysin_Breath — golden sine whose amplitude breathes between flat and tall
...
void mainImage(out vec4 C, in vec2 U){
    ...
    uv += fogWarp(uv, t);
    vec3 col=vec3(0.);
    float amp=.34*(.25+.75*abs(sin(.45*t)));
    float k=6.28318/1.4;
    float y=amp*sin(k*uv.x-1.2*t);
    float d=abs(uv.y-y);
    col+=vec3(1.,.84,.20)*(.012/(d+.004));
    col+=vec3(1.,.65,.05)*.010/(d*d*30.+.022);
    float y2=.5*amp*sin(2.*k*uv.x+.8*t+1.7);
